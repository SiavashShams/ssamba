...
if os.path.exists('./data/speech_commands_v0.01') == False:
    ...
    sc_url = 'https://storage.googleapis.com/download.tensorflow.org/data/speech_commands_v0.01.tar.gz'
    wget.download(sc_url, out='./data/')
    os.mkdir('./data/speech_commands_v0.01')
    os.system('tar -xzvf ./data/speech_commands_v0.01.tar.gz -C ./data/speech_commands_v0.01')
    #os.remove('./data/speech_commands_v0.01.tar.gz')

# generate training list = all samples - validation_list - testing_list
if os.path.exists('./data/speech_commands_v0.01/train_list.txt')==False:
    with open('./data/speech_commands_v0.01/validation_list.txt', 'r') as f:
        val_list = f.readlines()

    with open('./data/speech_commands_v0.01/testing_list.txt', 'r') as f:
    ...
    base_path = './data/speech_commands_v0.01/'
    ...
    with open('./data/speech_commands_v0.01/train_list.txt', 'w') as f:
...
#     with open('./data/SpeechCommands/speech_commands_v0.01/train_list.txt', 'w') as f:
...
if os.path.exists('./data/datafiles') == False:
    ...
    for split in ['testing', 'validation', 'train']:
        ...
        for file in filelist:
            ...
            cur_path = os.path.abspath(os.getcwd()) + '/data/speech_commands_v0.01/' + file.strip()
