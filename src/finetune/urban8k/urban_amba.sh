...
for val_fold in {1..10}; do
  ...
  CUDA_CACHE_DISABLE=1 python -W ignore ../../run_amba_1sec.py --use_wandb --dataset ${dataset} \
  ...
  map=$(tail -n 1 $exp_dir/metrics.csv | cut -d',' -f2) # Modify according to the actual column in your CSV
  acc=$(tail -n 1 $exp_dir/metrics.csv | cut -d',' -f3) # Modify according to the actual column in your CSV
