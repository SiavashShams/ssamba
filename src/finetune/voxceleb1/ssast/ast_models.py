...
class AMBAModel(nn.Module):
    def __init__(self, label_dim=527,
        ...
        # use a pretrained models for finetuning
            ...
            combined_vision_mamba_config = {**default_vision_mamba_config, **vision_mamba_config}
            # Replace self.v with MambaBlocksSequential
            print("combined_vision_mamba_config",default_vision_mamba_config)
    ...
    def finetuningavgtok(self, x):
        ...
        """
        if self.v.if_cls_token:
            if self.v.use_double_cls_token:
                hidden_states = (hidden_states[:, token_position[0], :] + hidden_states[:, token_position[1], :]) / 2
            else:
                hidden_states = hidden_states[:, token_position, :]
        else:
            if self.v.final_pool_type == 'none':
                hidden_states = hidden_states[:, -1, :]
            elif self.v.final_pool_type == 'mean':
                hidden_states = hidden_states.mean(dim=1)
            elif self.v.final_pool_type == 'max':
                hidden_states = hidden_states
            elif self.v.final_pool_type == 'all':
                hidden_states = hidden_states
            else:
    ...
    def mpc(self, x, mask_patch, cluster, show_mask=False):
        ...
        #x = hidden_states
        #print("after forward layers",x.shape)
        ...
        x = self.v.pos_drop(x)
        hidden_states = x
        residual = None  # Initial residual is None
        for blk in self.v.layers:
            x = blk(x)
    ...
    def forward(self, x, cluster=True, mask_patch=400):
        ...
        # mamba impl
        reps = []  # To store intermediate representations
        residual = None
        hidden_states = x
        token_position = 0
        if not self.v.if_bidirectional:
            for layer in self.v.layers:
                ...
                # Process and store the intermediate representation
        ...
        if not self.v.fused_add_norm:
            ...
            hidden_states = fused_add_norm_fn(
                ...
                residual_in_fp32=self.v.residual_in_fp32,
                        ...
                        # Process and store the intermediate representation for bidirectional case
        post_x = hidden_states[:, 1:, :]
        #print("Shape of post_x before reshaping:", post_x.shape)
        #print("Total elements in post_x:", post_x.nelement())
        post_x = (
                post_x.reshape(B, self.num_vertical_patch, -1, x.size(-1))
                .transpose(1, 2)
                .flatten(2)
        ...
        x = self.v.norm_f(hidden_states)
        return reps, x  # Return both the representations and the final output
